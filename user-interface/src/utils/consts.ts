export const consts = {
  titles: {
    edit: "ویرایش",
    delete: "حذف",
    exit: "خروج",
    show: "مشاهده",
    create: "ایجاد",
    cancel: "انصراف",
    toggleActivation: "تغییر وضعیت",
    revert: "بازگردانی",
    totalSelectedItem: "تعداد انتخاب شده ها",
    inActive: "غیرفعال",
    active: "فعال",
    yes: "بله",
    no: "خیر",
    dark: "تاریک",
    light: "روشن",
    system: "منطبق با سیستم",
    loginTitle: "شرکت سوده ایمن آرام",
    signIn: "ورود",
    forgotPassword: "فراموشی کلمه عبور",
    signUp: "ثبت نام",
    userName: "نام کاربری",
    password: "کلمه عبور",
    nationalCode: "کدملی",
    email: "ایمیل",
    phone: "همراه",
    forgotPasswordTitle: "بازیابی کلمه عبور",
    reset: "بازیابی",
    returnToLogin: "بازگشت به صفحه ورود",
    brandTitle: "شرکت سوده ایمن آرام",
    logout: "خروج",
    userCreate: "ایجاد کاربر",
    userEdit: "ویرایش کاربر",
    theme: "پوسته",
    updateRole: "بروزرسانی نقش",
    roles: "نقش ها",
    submit: "ثبت",
    description: "توضیحات",
    manageRoles: "مدیریت نقش",
    pagePermissions: "مدیریت دسترسی به صفحات",
    pages: "صفحات",
    updateUserPagePermission: "مدیریت دسترسی صفحات",
    personnelCreate: "ایجاد پرسنل",
    personnelEdit: "ویرایش پرسنل",
    activation: "فعال/غیرفعال",
    enterActivation: "فعال/غیرفعال را مشخص کنید",
    birthDate: "تاریخ تولد",
    name: "نام",
    family: "نام خانوادگی",
  },
  questions: {
    delete: "آیا برای حذف موارد انتخابی موافق هستید؟",
    revert: "آیا برای بازگردانی موارد انتخابی موافق هستید؟",
  },
  placeholders: {
    enterUserName: "نام کاربری را وارد نمایید",
    enterPassword: "کلمه عبور را وارد نمایید",
    enterNationalCode: "کدملی خود را وارد نمایید",
    enterEmail: "ایمیل را وارد نمایید",
    enterPhone: "همراه را وارد نمایید",
    chooseAnItem: "لطقا انتخاب نمایید...",
    enterBirthDate: "لطفا تاریخ تولد را وارد کنید...",
    enterName: "نام را وارد کنید",
    enterFamily: "نام خانوادگی را وارد کنید",
    enterActivation: "فعال یا غیرفعال را مشخص منید",
  },
  errors: {
    requiredUserName: "نام کاربری اجباریست",
    requiredName: "نام اجباریست",
    requiredFamily: "نام خانوادگی اجباریست",
    requiredBirthDate: "تاریخ تولد اجباریست",
    requiredPassword: " کلمه عبور اجباریست",
    requirednationalCode: " کدملی اجباریست",
    requiredEmail: " ایمیل اجباریست",
    requiredPhone: " همراه اجباریست",
    regexUserName: "فقط از حروف و اعداد انگلیسی استفاده نمایید",
    requiredRoles: "حداقل یک نقش باید انتخاب شده باشد",
  },
  messages: {
    serverNotRunning:
      "سرور سرویس دهنده در حال حاضر در دسترس نیست. لطفا با پشتیبان نرم افزاری خود تماس حاصل نمایید",
    toggleMessage:
      "لطفا برای تغییر وضعیت موارد انتخابی یکی از گزینه ها را انتخاب نمایید",
    successCreate: "عملیات ایجاد با موفقیت انجام شد",
    successEdit: "عملیات ویرایش با موفقیت انجام شد",
    successDelete: "عملیات حذف با موفقیت انجام شد",
    successUserRolesUpdate: "نقش کاربر با موفقیت بروزرسانی شد",
    emptyMessage: "داده ای برای نمایش وجود ندارد",
    successPagePermissionsUpdate:
      "سطح دسترسی صفحه مورد نظر با موفقیت تغییر یافت",
    successImageUploaded: "آپلود با موفقیت انجام شد",
  },
};
